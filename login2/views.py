...
from GnC.models import Goals, Competencies, Departmental_Goals, Departmental_Competencies, KPI
...
#Dashboard Rubrics
    ...
    rubric_overall_appraisal_database1 = Overall_Appraisal.objects.distinct().filter(user_appraisal_list__employee=request.user.profile ,status='Stage 1').count
...
#Department:
    ...
    subordinate_GCS_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 1', manager=request.user.profile)

    #Filtering to get all uncompleted UAL where manager = user (Subordinates)
    subordinate_midyear_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 1B', manager=request.user.profile)

    #Filtering to get all UAL with stage = 2 where manager = user (Subordinates)
    subordinate_appraisal_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 2', manager=request.user.profile)

    #Filtering to get all UAL with stage = 3 where manager = user (Subordinates)
    subordinate_reports_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 3', manager=request.user.profile)

    #Filtering to get all UAL with stage = 4 where manager = user (Subordinates)
    subordinate_calibration_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 4', manager=request.user.profile)
...
#Company:
    ...
    employee_calibration_list_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Stage 4')
...
#Company: Reports
    ...
    records_overall_appraisal_database = Overall_Appraisal.objects.filter(Q(status='Stage 3') | Q(status='Stage 4') | Q(status='Completed'))

#Employee Management
    #!Filtering to get all profiles excluding HR department
    company_profile_database = Profile.objects.exclude(department = request.user.profile.department)
...
#Employee Management
    #!Filtering to get all profiles excluding HR department
    company_profile_database = Profile.objects.all()
    #!Filtering to get all OA excluding completed ones
    company_appraisal_database=Overall_Appraisal.objects.exclude(status='Completed')
    #!Filtering to get all completed OAs
    company_completed_appraisal_database=Overall_Appraisal.objects.filter(status='Completed')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user_created:
                ...
                try:
                    email_string = PROFILE_CREATION_EMAIL(profile.name, userprofile.username, profile.email, profile.typeOfEmployee)
                    send_mail('Performance Management System Login Details', email_string, from_email = 'denselight_epmp@consulthunet.com', recipient_list=[profile.email], fail_silently=True)
                except Exception:
                    pass
...
def Detail_Profile(request, *args, **kwargs):
    ...
    records_user_appraisal_database = User_Appraisal_List.objects.filter(overall_appraisal__status__contains='Completed', employee = user_profile)
...
def Profile_Upload(request):
    ...
    for column in csv.reader(io_string, delimiter = ',', quotechar="|"):
        ...
        lowered_username = lowered_username.replace(' ', '')
        ...
        if departmentname != "":
            createddepartment, deptcreated = Departments.objects.update_or_create(name = departmentname)
        try:
        ...
        if deptcreated:
            userdepartment = createddepartment
        else:
            pass
        ...
        if user_created:
            profile_created, created = Profile.objects.create(
                user = profileuser,
        ...
        if created and user_created:
            try:
                email_string = PROFILE_CREATION_EMAIL(profile_created.name, profileuser.username, profile_created.email, profile_created.typeOfEmployee)
