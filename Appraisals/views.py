...
def User_Appraisals(request):
    context = {}
    return render(request, 'Appraisals/HuNet_Appraisals.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['Manager'])
def Manager_Appraisals(request):
    context = {}
    return render(request, 'Appraisals/HuNetM_Appraisals.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['HR'])
def HR_Appraisals(request):
    Overall_Appraisal_database = Overall_Appraisal.objects.all()

    context = {
        "Overall_Appraisal_list": Overall_Appraisal_database,

    }
    return render(request, 'Appraisals/HuNetRoot_Appraisals.html', context)
...
#------------------DetailView--------------------#
#
#class Detail_Appraisal_Category(DetailView):
#    model = Appraisal_Category
#    template_name = 'Appraisals/HuNet_Delete.html'
#    success_url = reverse_lazy#('Appraisals:User_Appraisals')
#
...
#
#class Detail_Rating_Scale(DetailView):
#    model = Rating_Scale
#    template_name = 'Appraisals/HuNet_Delete.html'
#    success_url = reverse_lazy#('Appraisals:User_Appraisals')
#
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class S2MtS2E_User_Appraisal_List(UpdateView):
    model = User_Appraisal_List
    form_class = AppraisalRejectionForm
    success_url = reverse_lazy('user_homepage')
    template_name = 'Appraisals/Update_S2MtS2E_UAL.html'
    ...
    def form_valid(self, form):
        form.instance.status = 'S2Employee'
        print(form.cleaned_data)
        return super(S2MtS2E_User_Appraisal_List, self).form_valid(form)
...
def Update_Appraisal(request, *args, **kwargs):
    ...
    form = UpdateUserAppRatingForm(request.POST or None, instance = selfappraisal)
    if form.is_valid():
        user_app = form.save(commit=False)
        user_app.completion = 'ECompleted'
        user_app.save()
        return HttpResponseRedirect(reverse('user_homepage'))
...
def UpdateAppraisalG_M(request, *args, **kwargs):
    ...
    GoalsFormset=modelformset_factory(Goals, form = MAppGoalsForm, extra = 0)
    queryset1=Goals.objects.filter(appraisal = selfappraisal, employee = selfappraisal.employee)
...
def UpdateAppraisalG_B(request, *args, **kwargs):
    ...
    GoalsFormset=modelformset_factory(Goals, form = BAppGoalsForm, extra = 0)
    queryset1=Goals.objects.filter(appraisal = selfappraisal, employee = selfappraisal.employee)
...
def Add_User_Appraisal_Dept(request, *args, **kwargs):
    id=kwargs.get('pk')
    overall_appraisal = Overall_Appraisal.objects.get(id=id)
    department_database = Departments.objects.all()
